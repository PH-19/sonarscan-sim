...
ABLATION_OPTIONS = {
    "BELIEF_PSO_NO_COVERAGE": {**DEFAULT_OPTIONS, "coverage_debt": False},
    "BELIEF_PSO_NO_UNCERTAINTY": {**DEFAULT_OPTIONS, "uncertainty": False},
    "BELIEF_PSO_FIXED_RANGE": {**DEFAULT_OPTIONS, "adaptive_range": False},
    "BELIEF_PSO_NO_PSO": {**DEFAULT_OPTIONS, "pso": False},
}
...
def _predict_position(track: dict, horizon_sec: float) -> dict:
...
def _eligible_sonars(snapshot: dict, track: dict) -> list[int]:
    result = []
    predicted = _predict_position(track, 1.0)
...
def _roi_geometry(
    ...
    predicted = [
        _predict_position(track, clamp(1.0 + 0.25 * track.get("timeSinceUpdate", 0.0), 1.0, 2.5))
        for track in tracks
    ]
    ...
    lo = min(angle - margin for angle, margin in zip(local_angles, angular_margins))
    hi = max(angle + margin for angle, margin in zip(local_angles, angular_margins))
    center = (lo + hi) / 2.0
    sector_width = sonar["maxLocalAngle"] - sonar["minLocalAngle"]
    width = clamp(hi - lo, MIN_ROI_DEG, min(MAX_ROI_DEG, sector_width))
    lo = clamp(center - width / 2, sonar["minLocalAngle"], sonar["maxLocalAngle"] - width)
    hi = lo + width
    if not options["adaptive_range"]:
        return lo, hi, max_range
    target_range = max(
    ...
    return lo, hi, clamp(target_range, 1.0, max_range)
...
def _track_scan_window(
    ...
    horizon = clamp(1.0 + 0.30 * age, 1.0, 4.0 if recovery else 2.5)
    predicted = _predict_position(track, horizon)
    ...
    return {
        "minLocalAngle": lo,
        "maxLocalAngle": hi,
        "range": clamp(scan_range, 1.0, max_range),
...
def _segmented_roi_plan(
    ...
    windows = [
        window for window in (
            _track_scan_window(sonar, track, max_range, options)
            for track in tracks
        )
    ...
    if not needs_segmentation:
        lo, hi, scan_range = _roi_geometry(sonar, tracks, max_range, options)
        return {
            "sonarId": sonar["id"],
            "minLocalAngle": lo,
            "maxLocalAngle": hi,
...
def _primary_assignment_cost(
    ...
    lo, hi, scan_range = _roi_geometry(sonar, candidate_group, max_range, options)
...
def _repair_assignments(
    ...
    for track, assignment in zip(tracks, assignments):
        ...
        corrected.append(assignment if assignment in eligible else min(
            eligible,
            key=lambda index: distance(sonars[index]["position"], _predict_position(track, 1.0)),
        ))
...
def _best_recovery_track_for_sonar(
    ...
    def score(track: dict) -> tuple[float, float, str]:
        predicted = _predict_position(track, clamp(1.0 + 0.3 * track.get("timeSinceUpdate", 0.0), 1.0, 4.0))
        range_term = distance(sonar["position"], predicted) / max(1.0, max_range)
        stickiness = 0.35 if track["id"] in previous else 0.0
        return (
...
def _assignment_cost(
    ...
    target_min = 1 if overloaded else 0
    target_max = max(1, math.ceil(len(tracks) / max(1, len(available))))
    ...
    for index, group in enumerate(groups):
        sonar = sonars[index]
        if not group:
            if index in available and target_min > 0:
            ...
            continue
        lo, hi, scan_range = _roi_geometry(sonar, group, max_range, options)
        duration = _scan_duration(snapshot, sonar, lo, hi, scan_range, TRACK_ROI_STEP_DEG)
        durations[index] = duration
        previous = set(sonar.get("assignedTargetIds") or [])
        current = {track["id"] for track in group}
        width = hi - lo
        excess_capacity = max(0, len(group) - target_max) if overloaded else 0
        ...
        for track in group:
            predicted = _predict_position(track, duration)
...
def _plan_with_options(snapshot: dict, strategy_id: str, options: dict) -> dict:
    ...
    for index, sonar in enumerate(sonars):
        if not groups[index]:
            ...
            continue
        plans.append(_segmented_roi_plan(sonar, groups[index], max_range, options))
...
def plan_no_coverage(snapshot: dict) -> dict:
    return _plan_with_options(snapshot, "BELIEF_PSO_NO_COVERAGE", ABLATION_OPTIONS["BELIEF_PSO_NO_COVERAGE"])


def plan_no_uncertainty(snapshot: dict) -> dict:
    return _plan_with_options(snapshot, "BELIEF_PSO_NO_UNCERTAINTY", ABLATION_OPTIONS["BELIEF_PSO_NO_UNCERTAINTY"])


def plan_fixed_range(snapshot: dict) -> dict:
    return _plan_with_options(snapshot, "BELIEF_PSO_FIXED_RANGE", ABLATION_OPTIONS["BELIEF_PSO_FIXED_RANGE"])


def plan_no_pso(snapshot: dict) -> dict:
    return _plan_with_options(snapshot, "BELIEF_PSO_NO_PSO", ABLATION_OPTIONS["BELIEF_PSO_NO_PSO"])
